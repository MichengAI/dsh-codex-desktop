...
try {
  ...
  assert.equal(await pet().webContents.executeJavaScript('typeof petWindow.onCursor'),'function','隔离窗口必须能接收主进程光标')
  await pet().webContents.executeJavaScript(`window.gazeSeen=false;petWindow.onCursor(value=>{if(value&&Number.isFinite(value.dx)&&Number.isFinite(value.dy))window.gazeSeen=true});void 0`)
  await read(`snapshot.notifications.activity={pose:'idle',title:'',text:''};publish()`)
  await until(()=>pet().webContents.executeJavaScript('gazeSeen'),'v2 空闲注视光标')
