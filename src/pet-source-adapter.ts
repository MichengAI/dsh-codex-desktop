...
export function installPetSourceAdapter(): void {
  type Snapshot = { pet: { id: string; url: string } | null; config: { visible: boolean }; language: string; notifications: { activity: unknown; items: Array<{ id: string; token: string }> } }
  type Api = { version: number; getSnapshot(): Snapshot | null; subscribe(fn: () => void): () => void; acquireDisplay(): () => void; command(value: unknown): Promise<void>; updateConfig(value: unknown): Promise<void>; openSettings(): void }
  type Bridge = { sync(value: unknown): Promise<void>; onCommand(fn: (value: unknown) => Promise<void>): () => void; onAction(fn: (value: string) => void): () => void }
  const page = window as unknown as { dshPet?: Api; dshPetHost?: Bridge; __disposePetAdapter?: () => void }
  ...
  const loadImage = async (pet: NonNullable<Snapshot['pet']>) => {
    ...
    const response = await fetch(pet.url, { credentials: 'same-origin', redirect: 'error', signal: AbortSignal.timeout(10000) })
    if (!response.ok) throw new Error(`宠物图片加载失败：${response.status}`)
    const blob = await response.blob()
    if (!['image/png', 'image/webp'].includes(blob.type) || blob.size > 16 * 1024 * 1024) throw new Error('宠物图片格式或大小无效')
  ...
  const update = () => { requested++; void pump() }
  const connect = () => {
    if (disposed) return
    off?.(); off = undefined; releaseDisplay()
    imageUrl = ''; imageData = ''
  ...
  const dispose = () => {
    if (disposed) return
    disposed = true; off?.(); releaseDisplay(); api = undefined; update()
    offCommand(); offAction()
    window.removeEventListener('dsh-pet-ready', connect); window.removeEventListener('dsh-pet-disposed', connect)
