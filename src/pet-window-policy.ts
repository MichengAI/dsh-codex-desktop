...
export interface PetNotifications {
  items: Array<{ id: string; token: string; pose: string; title: string; text: string; sessionId?: string; updatedAt: number; request?: { key: string; kind: string; toolName?: string; reason?: string; questions?: Array<{ id: string; question: string; detail?: string; multiSelect?: boolean; options?: Array<{ label: string; description?: string }> }> } }>
  hidden: number
...
export function parsePetWindowState(value: unknown): PetWindowState | null {
  ...
  const { pet, config, activity, notifications, language, sprite } = value as Partial<PetWindowState>
  if (sprite !== undefined && (typeof sprite !== 'string' || sprite.length > 23 * 1024 * 1024 || !/^data:image\/(png|webp);base64,[A-Za-z0-9+/]+={0,2}$/.test(sprite))) return null
  if (!pet || !config || !activity) return null
  if (language !== undefined && (typeof language !== "string" || !/^[a-zA-Z]{2,8}(?:-[a-zA-Z0-9]{1,8})*$/.test(language))) return null
  ...
  if (notifications !== undefined && !validNotifications(notifications)) return null
  return { sprite, language: language ?? 'zh', notifications, pet: { ...pet }, config: { selected: pet.id, visible: config.visible, size: config.size, position: null }, activity: { pose: activity.pose, title: activity.title.slice(0, 200), text: activity.text.slice(0, 300), sessionId: activity.sessionId } }
}
...
function validNotifications(value: PetNotifications): boolean {
  ...
  if (!value || !Array.isArray(value.items) || value.items.length > 500 || !Number.isInteger(value.hidden) || value.hidden < 0) return false
  if (JSON.stringify(value).length > 1000000) return false
  return value.items.every(item => item && text(item.id, 512) && text(item.token, 1024) && text(item.title) && text(item.text) && ['waiting','failed','review','running'].includes(item.pose) && Number.isFinite(item.updatedAt) && (!item.request || (
    text(item.request.key, 1024) && text(item.request.kind, 100) && (item.request.toolName === undefined || text(item.request.toolName)) && (item.request.reason === undefined || text(item.request.reason, 100000)) && (item.request.questions === undefined || (Array.isArray(item.request.questions) && item.request.questions.length <= 100 && item.request.questions.every(q => q && text(q.id, 512) && text(q.question) && (q.detail === undefined || text(q.detail, 100000)) && (q.multiSelect === undefined || typeof q.multiSelect === 'boolean') && (q.options === undefined || (Array.isArray(q.options) && q.options.length <= 100 && q.options.every(o => o && text(o.label) && (o.description === undefined || text(o.description)))))))))))
}
export function validPetCommand(value: unknown, state: PetWindowState | null): boolean {
  if (!value || typeof value !== 'object' || !state?.notifications) return false
  ...
  const item = state.notifications.items.find(n => n.id === v.id && n.token === v.token)
