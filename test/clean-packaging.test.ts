...
test('打包清理覆盖运行时残留和 release，不含用户 Profile', async () => {
  ...
  assert.match(source, /entry\.isDirectory\(\) && entry\.name\.startsWith\('release-'\)/)
...
test('清理只删除列出的残留目录，保留其他文件', async () => {
  ...
  try {
    ...
    await mkdir(join(root, 'release-1062'), { recursive: true })
    await writeFile(join(root, 'release-1062', 'app.zip'), 'zip', 'utf8')
    await writeFile(join(root, 'release-notes.md'), 'keep notes', 'utf8')
    await mkdir(join(root, 'src'), { recursive: true })
    await writeFile(join(root, 'src', 'main.ts'), 'keep', 'utf8')
    const cleaned = await cleanPackagingArtifacts(root)
    assert.deepEqual(cleaned.sort(), ['release', 'release-1062', 'runtime-plugins'].sort())
    assert.equal(existsSync(join(root, 'runtime-plugins')), false)
    assert.equal(existsSync(join(root, 'release')), false)
    assert.equal(existsSync(join(root, 'release-1062')), false)
    assert.equal(existsSync(join(root, 'release-notes.md')), true)
