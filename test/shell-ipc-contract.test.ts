...
test('sandbox preload 的 IPC 字面量与主契约保持一致', async () => {
  ...
  assert.deepEqual(channelLiterals(shell), [SHELL_IPC.action, SHELL_IPC.getBootstrap, SHELL_IPC.popupMenu, SHELL_IPC.state].sort())
  assert.deepEqual(channelLiterals(dsh), [SHELL_IPC.dshAction, SHELL_IPC.dshState].sort())
})
